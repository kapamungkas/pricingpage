...
function cleanCurrencySymbols(string $currencyString): string
{
    // Hapus simbol mata uang dan spasi di sekitarnya
    // Urutan penting: 'Rp. ' dan 'Rp ' harus duluan sebelum 'Rp' saja
    $cleanedString = str_replace(['Rp. ', 'Rp.', 'Rp ', '$ ', 'Rp', '$'], '', $currencyString);
    $cleanedString = trim($cleanedString); // Hapus spasi di awal/akhir jika ada
...
try {
    ...
    log_message("FATAL ERROR: Failed to load WordPress environment. " . $e->getMessage());
...
try {
    ...
    log_message("ERROR: Failed to receive or decode webhook payload. " . $e->getMessage());
    die('Failed to receive or decode webhook payload.');
}
...
$internalSQM = '';
$externalSQM = '';
$order_usd_to_idr = 0; // Default value
        ...
        foreach ($line_item['meta_data'] as $meta) {
            switch ($meta['key']) {
                ...
                case 'order_price_IDR':
                    $price_idr = cleanCurrencySymbols($meta['value']);
                    break;
                case 'order_price_USD':
                    $price_usd = cleanCurrencySymbols($meta['value']);
                    break;
                case 'order_fee_IDR':
                    $booking_fee = cleanCurrencySymbols($meta['value']);
                    break;
                ...
                case 'order_externalSQM':
                    ...
                    break;
                case 'order_usd_to_idr':
                    $order_usd_to_idr = $meta['value'];
                    break;
            }
        }
    }
...
$order_usd_to_idr_raw = $order_usd_to_idr; // This is your example input string

// --- Conversion Logic ---
// 1. Remove thousands separators (dots)
$cleaned_value = str_replace('.', '', $order_usd_to_idr_raw);
// 2. Replace the decimal comma with a decimal dot
$cleaned_value = str_replace(',', '.', $cleaned_value);
// 3. Convert to float
$order_usd_to_idr = (float) $cleaned_value;
...
$billing_extension_idr = 0;
$billing_extension_usd = 0;



try {
    foreach ($meta_data as $meta) {
        // Ensure 'key' and 'value' exist before accessing to prevent undefined index notices
        if (isset($meta['key']) && isset($meta['value'])) {
            switch ($meta['key']) {
                ...
                case 'billing_passport_number':
                    ...
                    break;
                case '_billing_lease_extension':
                case 'billing_lease_extension':
                    // Make sure $order_usd_to_idr is defined and is a numeric value
                    if (isset($order_usd_to_idr) && is_numeric($order_usd_to_idr)) {
                        if ($meta['value'] == 'during_construction') {
                            $billing_extension_idr = number_format(19000 * floor($order_usd_to_idr)); // IDR
                            $billing_extension_usd = number_format(19000); // USD
                        } elseif ($meta['value'] == 'within_2_years_after_handover') {
                            $billing_extension_idr = number_format(24700 * floor($order_usd_to_idr)); // IDR
                            $billing_extension_usd = number_format(24700); // USD
                        }
                    } else {
                        log_message("Warning: \$order_usd_to_idr is not defined or not numeric for lease extension calculation.");
                    }
                    break;
            }
        } else {
            log_message("Warning: Meta data item missing 'key' or 'value' field.");
        }
    }
    log_message("Extracted additional billing info from meta_data successfully.");
} catch (Throwable $e) {
    // Catch any type of error (Exception or Error)
    log_message("Error extracting additional billing info from meta_data: " . $e->getMessage());
    // You might want to set default values or handle the error in another way
}
...
$apiKey = '20a36a45ecd1ef7ebdbdd71c32bbf21d22b7d326'; // It's generally better to store API keys securely, e.g., in environment variables.
...
$dataCreateDocument = [
    ...
    "template_uuid" => "WnDc92B5YQxykpWB8DEHxR", // PandaDoc Template UUID
    "recipients" => [
        [
    ...
    "tokens" => [
        ...
        ["name" => "Unit.Extension.Price.IDR", "value" => $billing_extension_idr],
        ["name" => "Unit.Extension.Price.USD", "value" => $billing_extension_usd]
    ],
    "metadata" => [
        "order_id" => $order_id,
...
if ($curlCreateDocument === false) {
    log_message("FATAL ERROR: Failed to initialize cURL for document creation.");
...
try {
    ...
    if ($responseCreateDocument === false) {
        throw new Exception("cURL error when creating document: " . curl_error($curlCreateDocument));
    }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        throw new Exception("JSON decoding error for create document response: " . json_last_error_msg());
    }
    ...
    log_message("ERROR: Exception during document creation. " . $e->getMessage());
    if ($curlCreateDocument !== false) {
        curl_close($curlCreateDocument);
    }
...
function checkDocumentStatus($documentId, $apiKey, $cookie) {
    $curl = curl_init();
    if ($curl === false) {
        log_message("FATAL ERROR: Failed to initialize cURL for document status check.");
    ...
    try {
        ...
        if ($response === false) {
            throw new Exception("cURL error when checking document status: " . curl_error($curl));
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception("JSON decoding error for document status response: " . json_last_error_msg());
        }
        return $decodedResponse['status'];
    } catch (Exception $e) {
        log_message("ERROR: Exception during document status check. " . $e->getMessage());
        if ($curl !== false) {
            curl_close($curl);
        }
        return false; // Indicate failure
...
while ($status !== 'document.draft' && $attempt < 30) {
    ...
    if ($status === false) {
        die('Error checking document status. See logs for details.');
    }
...
// Send the document
$dataSendDocument = [
    "message" => "Hello " . ($billing['first_name'] ?? 'Client') . "! This is your booking document for Unit " . $unit_number . ". Please review and sign the document.",
    "silent" => true
];

$curlSend = curl_init();
if ($curlSend === false) {
    log_message("FATAL ERROR: Failed to initialize cURL for sending document.");
    die('Failed to initialize cURL.');
}
try {
    ...
    if ($responseSend === false) {
        throw new Exception("cURL error when sending document: " . curl_error($curlSend));
    }
    ...
    log_message("ERROR: Exception during document sending. " . $e->getMessage());
    if ($curlSend !== false) {
        curl_close($curlSend);
    }
    die('Failed to send document: ' . $e->getMessage());
}
...
if (json_last_error() !== JSON_ERROR_NONE) {
    log_message("WARNING: JSON decoding error for send document response: " . json_last_error_msg());
} else {
...
try {
    ...
    log_message("ERROR: Exception during order meta update or email sending. " . $e->getMessage());
    echo "Error updating order or sending email: " . $e->getMessage() . "\n";
}
